...
$.ajax({
    ...
    data: {
        key: 'L3FBZ-W7JLX-6JG4O-Z73EX-JQVRO-Q4BRU',
        output: 'jsonp',
    },
    dataType: 'jsonp',
...
function showWelcome() {
    ...
    else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>晚上好</span>，夜生活开始emo吧！";
    else timeChange = "夜深了，早点休息，少熬夜";

    //自定义文本需要放的位置
    document.getElementById("welcome-info").innerHTML = `欢迎来自<span>${pos}</span>的小伙伴，${timeChange}<br>你距离十七约有<span>${dist}</span>公里，${posdesc}`;
    document.getElementById("sidebar-welcome-info").innerHTML = `欢迎来自<span>${pos}</span>的小伙伴，${timeChange}<br>你距离十七约有<span>${dist}</span>公里，${posdesc}`;
}
